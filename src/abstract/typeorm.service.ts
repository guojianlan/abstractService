...
export abstract class AbstractTypeOrmService<T> {
  ...
  constructor(model: Repository<T>, _entity: EntityTarget<T>, options: AbstractServiceExtraOptions = {
  ...
  public async findOne(id: number, query?: any): Promise<T | boolean> {
    try {
      ...
      if (this.options && this.options.findInjectDeleteWhere) {
        this.addDeleteConditon(builder)
      }
  ...
  public async deleteLogSql(sql: any) {
    try {
      ...
      let result = await mangager.query(`CREATE TABLE if not exists ${create_table_name} like ${origin_table_name}`)
      if (result) {
        await mangager.createQueryBuilder().insert().into(create_table_name).values(sql).execute()
        // 成功后添加记录
      }
      console.log('记录成功')
    } catch (error) {
